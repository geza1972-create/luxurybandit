...
import { ChevronLeft, Heart, Image as ImageIcon, Send } from "lucide-react";
...
type UserGalleryPayload = {
  userGallery?: GalleryItem[];
  displayName?: string;
};

// Seeded engagement for each generated image
...
export default function UserGalleryPage() {
  ...
  useEffect(() => {
    if (!username) return;
    setIsLoading(true);
    fetch(`/api/try-this-look?username=${encodeURIComponent(username)}`)
      .then(r => r.json())
      .then((p: UserGalleryPayload) => {
        setItems(p.userGallery ?? []);
  ...
  const shareProfile = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: `${displayName} on LuxuryBandit`, url }); } catch { /**/ }
    } else {
      try {
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <div className="flex flex-1 items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] font-bold text-black/40">AI Fashion Creator</p>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-white" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
          ...
          <div className="flex items-center gap-3 border-b border-black/10 px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {selected.storeName && (
                <p className="truncate text-[10px] font-bold text-black/40">{selected.storeName}</p>
              )}
            </div>
            <button type="button"
              onClick={() => router.push(`/look/${selected.lookId}`)}
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-2 gap-3">
              {selected.userPhotoUrl && (
                <div className="grid gap-1">
                  <div className="aspect-[3/4] overflow-hidden rounded-xl border border-black/10 bg-black/5">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={selected.userPhotoUrl} alt="Before"
                      className="h-full w-full object-cover object-top" />
                  </div>
              ...
              <div className={`grid gap-1 ${!selected.userPhotoUrl ? "col-span-2" : ""}`}>
                <div className="aspect-[3/4] overflow-hidden rounded-xl border border-black/10 bg-black/5">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={selected.imageUrl} alt="After"
                    className="h-full w-full object-cover object-top" />
                </div>
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <button type="button"
                onClick={async () => {
                  ...
                  if (navigator.share) {
                    try { await navigator.share({ title: selected.lookName, url }); } catch { /**/ }
                  } else {
