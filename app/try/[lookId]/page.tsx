...
const PROMPT_POOL = [
  ...
  "On golden desert sand dunes under a warm sunset sky, @1 presents the @2. She turns slowly all the way around to show it from every side, then walks a few relaxed steps toward the camera. Keep @1 face and appearance and the @2 exactly the same throughout. Fluid natural motion, photorealistic, high-end look. No text or logos.",
              ...
              <div className="mx-auto mt-3 w-fit overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04]">
                ...
                <div className="relative aspect-[9/16] h-[38vh] max-w-[78vw]">
                  ...
                  {chosenModelId && chosenModelName && !avatar && (
                    <button type="button" onClick={() => chosenModelId && router.push(`/mai-ieftin?model=${encodeURIComponent(chosenModelId)}&lang=${lang}`)} title={`Chat with ${chosenModelName.split(/\s+/)[0]}`}
                ...
                <div className="w-[42%] max-w-[160px]">
                  <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]">
                    ...
                    {chosenModelLocked ? (
                      ...
                      <button type="button" onClick={() => chosenModelId && router.push(`/mai-ieftin?model=${encodeURIComponent(chosenModelId)}&lang=${lang}`)} className="lb-gold absolute right-1.5 top-1.5 flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-black shadow"><MessageCircle className="h-3.5 w-3.5" /> AI Chat</button>
                    )}
                  </div>
                </div>
              </div>
                        ...
                        <div key={m.id} onClick={() => { if (swipedRef.current) { swipedRef.current = false; return; } if (isUpload) { if (!isActive) { setFront(m); return; } (isPaid || adminProduce) ? fileRef.current?.click() : setShowPremium(true); return; } if (!isActive) setFront(m); }}
                          ...
                          <div className="relative aspect-[3/4] w-full">
                            ...
                            {isActive && !m.realModel && chosenModelName && (
                              ...
                              <button type="button" onClick={(e) => { e.stopPropagation(); chosenModelId && router.push(`/mai-ieftin?model=${encodeURIComponent(chosenModelId)}&lang=${lang}`); }} title={`Chat with ${chosenModelName.split(/\s+/)[0]}`}
                                className="lb-gold absolute right-2 top-2 flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[12px] font-black shadow-lg active:scale-95 transition">
                                <MessageCircle className="h-4 w-4" /> AI Chat
                              </button>
                            )}
