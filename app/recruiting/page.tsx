...
export async function generateMetadata({ searchParams }: {
  ...
  const T = recruitingTexte(String(sp.lang ?? "") || (await resolveLang("de")));
  return { title: `LB Recruiting — ${T.positionierung}`, description: T.lead };
}

/**
 * DAS BEISPIELPROFIL (Owner 31.08.2026: „damit Recruiter sofort sehen, was sie von uns
 * bekommen").
 *
 * ES IST AUSDRÜCKLICH EIN BEISPIEL, KEIN KANDIDAT. Die Zeile darunter sagt das, und sie ist
 * nicht Kleingedrucktes aus Vorsicht, sondern die Regel des Hauses: Ein echtes Profil geht
 * nur mit Zustimmung dieses einen Menschen an diese eine Firma. Ein erfundener Kandidat, der
 * wie ein echter aussähe, wäre der schnellste Weg, das Vertrauen zu verlieren, das diese
 * Seite gerade aufbaut.
 *
 * Die Felder sind dieselben, die der Kandidaten-Trichter tatsächlich erhebt — Sprachniveau,
 * Ort, Arbeitsmodell, Wechselbereitschaft, Gehaltswunsch, Interesse. Was hier steht, ist
 * also keine Wunschliste, sondern die Form, in der ein Profil wirklich ankommt.
 */
...
export default async function RecruitingSeite({ searchParams }: {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
}) {
  /* Die Adresse sticht die gespeicherte Wahl — damit ein gezielt verschickter Link
     (`/recruiting?lang=ro`) bei jedem Empfänger gleich ankommt. Ohne Angabe: die Wahl des
     Nutzers, dann seine Browsersprache, dann Deutsch. */
  const sp = (await searchParams) ?? {};
  const ausAdresse = String(sp.lang ?? "");
  const lang = ausAdresse || (await resolveLang("de"));
  ...
  return (
    <main className="lb-bg min-h-screen text-white">
      ...
      <TopNav schlicht sprachen={RECRUITING_SPRACHEN} back={false} marke="LB Recruiting" heim="/recruiting" motto={T.kopfMotto} />
