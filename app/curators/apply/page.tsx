    ...
    return (
      <div className="grid min-h-[100dvh] place-items-center bg-[#faf7f0] px-6 text-center text-ink">
        <div className="max-w-sm">
          ...
          {appliedFaceId && (
            faceReserved ? (
              <p className="mt-4 rounded-2xl border border-emerald-400/30 bg-emerald-400/[0.08] px-4 py-3 text-[15px] font-black text-emerald-700">✓ Your face is reserved — it&apos;s yours alone. 💛</p>
            ) : (
              <div className="mt-4">
    ...
    <div className="min-h-[100dvh] bg-[#faf7f0] text-ink" style={{ paddingBottom: "calc(130px + env(safe-area-inset-bottom))" }}>
      ...
      <div className="px-5 pt-6">
        ...
        <div className="mt-6 rounded-2xl border border-amber-400/30 bg-amber-400/[0.07] px-4 py-3 text-center">
          <p className="text-[15px] font-black text-amber-700">⚠️ You get one shot — send your very best photos.</p>
          ...
          <p className="mt-2 text-[13px] font-bold leading-relaxed text-amber-700">🎬 Your first video is free — turn a photo into a video once you&apos;re approved. Every extra video is just $3.99.</p>
        ...
        <div className="mt-5 rounded-2xl border border-black/22 bg-black/[0.03] p-4">
          ...
          <div className="mt-1 grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setImageSource("own")}
              className={`rounded-xl border px-3 py-2.5 text-center text-[14px] font-black transition ${imageSource === "own" ? "border-amber-400 bg-amber-400/10 text-amber-700" : "border-black/25 text-ink/72"}`}>
            ...
            <button type="button" onClick={() => setImageSource("ours")}
              className={`rounded-xl border px-3 py-2.5 text-center text-[14px] font-black transition ${imageSource === "ours" ? "border-amber-400 bg-amber-400/10 text-amber-700" : "border-black/25 text-ink/72"}`}>
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                {avatarFaces.map(f => (
                  <button key={f.id} type="button" disabled={f.claimed}
                    ...
                    <span className={`absolute bottom-1 left-1 rounded-full px-1.5 py-0.5 text-[11px] font-black ${f.claimed ? "bg-black/70 text-ink/72" : "bg-emerald-500 text-ink"}`}>{f.claimed ? "Booked" : "Free · $3.99"}</span>
                    {avatarFaceId === f.id && <span className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-amber-400 text-[13px] font-black text-black">✓</span>}
                  </button>
                ))}
              </div>
            )}
          ...
          {(() => {
            ...
            const slot = (item: { src: string; isNew: boolean } | undefined, sizeCls: string, isMain: boolean) => item ? (
              <div className="relative">
                ...
                {isMain && <span className="pointer-events-none absolute bottom-1 left-1/2 -translate-x-1/2 rounded-full bg-amber-400 px-2 py-px text-[10px] font-black uppercase tracking-wide text-black">Main</span>}
              </div>
            ) : (
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {mottoIdeas.map((m, i) => (
                  <button key={i} type="button" onClick={() => setMotto(m)}
                    className={`flex items-center gap-1 rounded-full border px-3 py-1.5 text-sm font-black transition ${motto === m ? "border-amber-400 bg-amber-400 text-black" : "border-black/15 bg-black/[0.04] text-ink/80"}`}>
