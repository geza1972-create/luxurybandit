...
import { filmPfad, filmPosterPfad, wandFilmPfad, wandFilmPosterPfad, sprecherPfad, sprecherWebmPfad, sprecherBildPfad, sprecherTonPfad } from "@/lib/lakatosbandi-film";
...
export async function GET(request: Request) {
  ...
  const pfad = art === "sprecher" ? sprecherPfad(mandant, nr)
    ...
    /* Der zweite Film am Werk — das Blatt wird aufgehängt — und sein Standbild (20.09.2026). */
    : art === "wand" ? wandFilmPfad(mandant, nr)
    : art === "wandposter" ? wandFilmPosterPfad(mandant, nr)
    : filmPfad(mandant, nr);
  ...
  return new NextResponse(await res.arrayBuffer(), {
    headers: {
      "Content-Type": art === "sprecherbild" || art === "filmposter" || art === "wandposter" ? "image/jpeg"
