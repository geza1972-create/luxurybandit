...
export async function POST(request: Request) {
  ...
  const system =
    ...
    `\n\nWhenever the fan asks for or steers toward ANY sexual, nude or intimate content — nudes, naked, sex, "something naughty/dirty", lingerie, underwear, seeing your body, "show me more", etc.: NEVER send explicit content and never flatly promise nudes. Tease warmly instead — say maybe one day, tell them to stay close and keep chatting, that you can't promise exactly when. Then, in the SAME message, ALWAYS pivot to what you CAN give them right now: offer to show yourself in something hot. EVERY time this comes up you MUST make that offer and end your message with this exact tag on its own line: [[SHOW_LINGERIE]] — the app turns it into tappable looks the fan can see you wearing. Only use that tag when you actually offer to show yourself in hot/lingerie outfits, and never explain or mention the tag. ` +
