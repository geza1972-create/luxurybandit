...
// Allowed contact reasons (value → label). Anything else is rejected.
...
export async function POST(request: Request) {
  ...
  if (!REASONS[reason]) return NextResponse.json({ error: "Please choose a reason." }, { status: 400 });
  ...
  const supportHtml = `
    <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;font-size:15px;color:#111">
      <h2 style="margin:0 0 12px">New contact message · ${esc(reasonLabel)}</h2>
  ...
  if (!supportRes.ok) {
    return NextResponse.json({ error: "Could not send right now. Please try again later." }, { status: 502 });
  }
  ...
  const confirmHtml = `
    <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;font-size:15px;color:#111;max-width:520px">
      <h2 style="margin:0 0 12px">Thanks for your message, ${esc(name)}!</h2>
      <p style="margin:0 0 10px">We&apos;ve received your message and will get back to you as soon as possible.</p>
      <p style="margin:0 0 4px;color:#555"><strong>Reason:</strong> ${esc(reasonLabel)}</p>
