...
const TEXT: Record<string, (name: string, link: string, stop: string) => string> = {
  en: (n, l, u) => `Good morning${n ? ` ${n}` : ""}! Your weather + a new look from Bella: ${l} — {price} a month instead of {list}, forever. Stop: ${u}`,
  de: (n, l, u) => `Guten Morgen${n ? ` ${n}` : ""}! Dein Wetter + ein neuer Look von Bella: ${l} — dauerhaft {price} statt {list}. Abmelden: ${u}`,
  ro: (n, l, u) => `Bună dimineața${n ? ` ${n}` : ""}! Vremea ta + un look nou de la Bella: ${l} — permanent {price} în loc de {list}. Dezabonare: ${u}`,
  es: (n, l, u) => `¡Buenos días${n ? ` ${n}` : ""}! Tu clima + un look nuevo de Bella: ${l} — siempre {price} en vez de {list}. Baja: ${u}`,
  fr: (n, l, u) => `Bonjour${n ? ` ${n}` : ""} ! Ta météo + un nouveau look de Bella : ${l} — toujours {price} au lieu de {list}. Stop : ${u}`,
  pt: (n, l, u) => `Bom dia${n ? ` ${n}` : ""}! O teu tempo + um novo visual da Bella: ${l} — sempre {price} em vez de {list}. Cancelar: ${u}`,
  pl: (n, l, u) => `Dzień dobry${n ? ` ${n}` : ""}! Twoja pogoda + nowy look od Belli: ${l} — zawsze {price} zamiast {list}. Wypisz się: ${u}`,
  it: (n, l, u) => `Buongiorno${n ? ` ${n}` : ""}! Il tuo meteo + un look nuovo di Bella: ${l} — sempre {price} invece di {list}. Disiscriviti: ${u}`,
};
