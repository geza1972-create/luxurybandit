...
export default function CreatorProfilePage() {
  ...
  const creatorSlug = toSlug(String(params?.creator ?? ""));
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/profile/${encodeURIComponent(creatorSlug)}`).then(r => r.ok ? r.json() : null),
      fetch(`/api/try-this-look?username=${encodeURIComponent(creatorSlug)}`).then(r => r.ok ? r.json() : {}),
      fetch(`/api/follow?slug=${encodeURIComponent(creatorSlug)}&type=user`, { headers: authHeaders }).then(r => r.ok ? r.json() : { followerCount: 0, following: false }),
    ]).then(async ([prof, galleryData, followData]) => {
      const p = prof as ProfileData | null;
      setProfile(p ?? { userId: "", username: creatorSlug, displayName: galleryData?.displayName ?? creatorSlug });
      setTryons((galleryData?.userGallery ?? []) as TryonItem[]);
      ...
      // Load store looks if they have a store
      if (p?.storeSlug) {
      ...
      setProfile({ userId: "", username: creatorSlug, displayName: creatorSlug });
  ...
  const handleFollow = async () => {
    ...
    try {
      const res = await fetch("/api/follow", {
        ...
        body: JSON.stringify({ slug: creatorSlug, type: "user", action: following ? "unfollow" : "follow" }),
      });
      if (res.ok) {
  ...
  const handleSendMsg = async () => {
    ...
    try {
      const res = await fetch("/api/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ toUserId: profile?.userId, toSlug: creatorSlug, message: msgText.trim() }),
      });
  ...
  const displayName = profile.displayName || creatorSlug;
  const totalCreator = looks.length;
      ...
      <div className="min-h-screen bg-white" style={{ paddingBottom: "calc(3.5rem + env(safe-area-inset-bottom))" }}>
        ...
        <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-black/8">
          <div className="flex items-center h-12 px-3 max-w-lg mx-auto">
            ...
            <span className="flex-1 text-center text-sm font-black text-black truncate px-2">
              {displayName}
            </span>
            <div className="w-9" />
          </div>
        ...
        <div className="max-w-lg mx-auto">
          ...
          <div className="px-5 pt-6 pb-4">
              ...
              <div className="flex-1 pt-1">
                <div className="grid grid-cols-3 gap-1 text-center">
                  <div className="flex flex-col">
                    <span className="text-lg font-black text-black leading-tight">{totalCreator}</span>
                    <span className="text-[11px] font-bold text-black/50 leading-tight">Looks</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-lg font-black text-black leading-tight">{totalTryons}</span>
                    <span className="text-[11px] font-bold text-black/50 leading-tight">Tryons</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-lg font-black text-black leading-tight">{fmtN(followerCount)}</span>
                    <span className="text-[11px] font-bold text-black/50 leading-tight">Follower</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Name + bio */}
            <div className="mt-3">
              <p className="text-sm font-black text-black leading-snug">{displayName}</p>
              {profile.bio && (
                <p className="mt-1 text-[13px] font-medium text-black/70 leading-snug whitespace-pre-line">
                  {profile.bio}
                </p>
              )}
              {/* Social links */}
              <div className="mt-2 flex flex-wrap gap-3">
                {profile.website && (
                  <a href={profile.website.startsWith("http") ? profile.website : `https://${profile.website}`}
                    ...
                    className="flex items-center gap-1 text-[12px] font-bold text-black/60 hover:text-black transition">
            ...
            <div className="mt-4 flex gap-2">
              <button
                ...
                className={`flex-1 h-9 rounded-xl text-sm font-black flex items-center justify-center gap-1.5 transition
                  ${following
                    ? "border-2 border-black/20 text-black"
                    : "bg-black text-white"
                  }`}
              >
              ...
              <button
                type="button"
                onClick={() => { setShowMsg(true); setSent(false); setMsgErr(""); }}
                className="flex-1 h-9 rounded-xl border-2 border-black/15 text-sm font-black text-black flex items-center justify-center gap-1.5 transition active:bg-black/5"
              >
          ...
          <div className="grid grid-cols-2 border-t border-b border-black/8">
            <button
              ...
              className={`py-3 text-sm font-black transition border-b-2 ${
                activeTab === "creator"
              ...
              {looks.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-px bg-black/8 border-t border-black/8">
                  {looks.map((look) => {
                    ...
                    const likeCount = genLikes(look.id) + (liked ? 1 : 0);
                    return (
                      <button
                        key={look.id}
                        type="button"
                      ...
                      >
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent p-2.5">
                          <p className="text-white text-[11px] font-black leading-tight line-clamp-1">{look.name}</p>
                          {look.price && (
                            <p className="text-white/90 text-[11px] font-bold">{look.price}</p>
                          )}
                        </div>
                        {/* Like */}
                        <button
                        ...
                        >
                          <Heart
                            className={`h-4 w-4 drop-shadow transition ${liked ? "fill-red-500 text-red-500" : "text-white"}`}
                          />
                          <span className="text-white text-[10px] font-black drop-shadow">{fmtN(likeCount)}</span>
                        </button>
                        {/* Out of stock badge */}
                        {look.inStock === false && (
              ...
              {tryons.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-px bg-black/8 border-t border-black/8">
                  {tryons.map((item) => {
                    ...
                    const likeCount = genLikes(item.id) + (liked ? 1 : 0);
                    return (
                      <button
                        key={item.id}
                        type="button"
                      ...
                      >
                        ...
                        {/* Overlay */}
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent p-2.5">
                          <p className="text-white text-[11px] font-black leading-tight line-clamp-1">{item.lookName}</p>
                          {item.storeName && (
                            <p className="text-white/70 text-[10px]">{item.storeName}</p>
                          )}
                        </div>
                        {/* Like */}
                        <button
        ...
        <>
          <div className="fixed inset-0 z-50 bg-black/90 flex flex-col" onClick={() => setLightbox(null)}>
            ...
            <div className="flex-1 flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
              <img src={lightbox.imageUrl} alt={lightbox.lookName}
          ...
          <div className="fixed inset-x-0 bottom-0 z-[51] rounded-t-2xl bg-white shadow-2xl px-5 pt-4"
            ...
            {sent ? (
              <div className="py-6 text-center">
                ...
                <button type="button" onClick={() => setShowMsg(false)}
                  className="mt-4 text-sm font-bold text-black/40">Schließen</button>
              </div>
            ) : (
              <>
                <textarea
                ...
                <button
                  ...
                  className="mt-3 w-full h-11 rounded-xl bg-black text-white text-sm font-black flex items-center justify-center gap-2 disabled:opacity-40 transition"
                >
