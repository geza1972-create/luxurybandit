// Lingerie / intimates category — TASTEFUL pieces only (slip dresses, robes,
// kimonos, bodysuits, loungewear). Pulls REAL product images via /api/discover,
// creates shoppable looks, and generates a tasteful try-on per look. NO video,
// and NO revealing bra/brief sets (the search queries target tasteful pieces).
//   TRY_THIS_LOOK_ADMIN_PIN=… node scripts/seed-lingerie.mjs
...
const OWNER = "curator-1782204766325-wfyi0"; // Szidonia — owns the looks, bears credits

// curators who wear the pieces (id + photo)
const WEARERS = [
...
// brand → tasteful search query; 2 heroes taken per brand
const BRANDS = [
  { brand: "Intimissimi",  query: "Intimissimi silk modal slip dress women" },
  { brand: "Hunkemöller",  query: "Hunkemoller satin kimono robe loungewear women" },
  { brand: "Triumph",      query: "Triumph shaping bodysuit body women" },
  { brand: "La Perla",     query: "La Perla silk slip dress robe women" },
];
// only keep tasteful garment types; skip anything that reads as a bra/brief set
const TASTEFUL = /(slip|dress|robe|kimono|gown|bodysuit|body|cami|loungewear|nightdress|negligee|chemise)/i;
const SKIP = /(thong|brief|panty|panties|g-string|bra set|push-up|bralette set)/i;
...
async function makeLook(brand, hero, rest) {
  ...
  const res = await fetch(`${BASE}/api/try-this-look`, { method: "POST", headers: { "Content-Type": "application/json", "x-try-look-admin-pin": PIN }, body: JSON.stringify({
    ...
    productType: "real", curatorId: OWNER, hashtags: `#${brand.replace(/[^a-z0-9]/ig, "")} #intimates #loungewear`,
  }) });
...
async function tryOn(look, wearer) {
  ...
  fd.append("prompt", `Tasteful editorial loungewear/intimates try-on for a fashion catalogue. Show the person wearing the selected piece in a clean, elegant studio, composed and non-sexualised. Preserve the person's face, hair, skin tone and identity exactly. Full-length, refined framing. Piece: ${look.name}.`);
...
async function main() {
  ...
  console.log(`Lingerie category → ${BASE}`);
  ...
  for (const b of BRANDS) {
    console.log(`\n=== ${b.brand} ===`);
    const items = (await discover(b.query)).filter(it => TASTEFUL.test(it.title) && !SKIP.test(it.title));
    if (!items.length) { console.log("  ✗ no tasteful products"); continue; }
    const heroes = items.slice(0, 2); // up to 2 looks per brand
    for (let i = 0; i < heroes.length; i++) {
      const hero = heroes[i];
      const rest = items.filter(x => x.link !== hero.link);
      try {
        const look = await makeLook(b.brand, hero, rest);
        if (look) await tryOn(look, WEARERS[w % WEARERS.length]);
        w++;
      } catch (e) { console.log("    ✗ error:", e.message); }
    }
  }
  console.log("\nLingerie category done.");
}
