...
export const KARTE_TEXTE: Record<string, {
  ...
  videoDaraus: string;
  /**
   * DIE WAHL VOR DEM ERZEUGEN (Owner 04.08.2026): Bild {once} oder gleich Video {videoauf}.
   * Das Bild ist der Regelfall — es kostet ein Drittel und die Einladung ist damit fertig.
   */
  ctaBild: string; ctaVideoGleich: string;
  /**
   * DER KAUFKNOPF DES GUTSCHEINS. `ctaBild` heisst „Einladung mit Bild" — beim Gutschein wird
   * weder eine Einladung noch ein Bild gemacht: Er bringt sein Video mit oder nimmt unseres,
   * und heraus kommt eine KARTE. Ein Knopf, der etwas anderes verspricht als das, was passiert,
   * kostet mehr als ein hässlicher.
   */
  ctaGutschein: string;
  /**
...
export default function EinladungKarte({
  ...
  /** Steht unter dem Bild noch etwas? Botschaft, Datum, Ort, Nummer oder die Herkunftszeile. */
  const hatUnten = !!(tag || ort || adresse || telefon || fuss || botschaft || aufBotschaft || von || aufVon || aufDatum || aufOrt);
