...
export default function DailySignupForm({ lang = "de" }: { lang?: "de" | "en" }) {
  ...
  const field = "h-13 w-full rounded-xl border-[1.5px] border-slate-400 bg-white px-4 py-3 text-[15px] font-bold text-slate-900 outline-none focus:border-slate-700 placeholder:text-slate-400";

  return (
    <div className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
      ...
      <button type="button" onClick={() => void submit()} disabled={busy}
        className="mt-3 flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-slate-800 text-base font-black text-white shadow-[0_3px_0_#0f172a,0_6px_14px_rgba(15,23,42,0.28)] transition active:translate-y-[3px] disabled:opacity-50">
        {busy ? <><Loader2 className="h-5 w-5 animate-spin" /> {t.busy}</> : t.cta}
      </button>
      <p className="mt-2 text-center text-[12px] font-bold text-slate-500">{t.note}</p>
    </div>
  );
}
