...
export default function BellaCarouselAdmin({ heading = "🎴 Card Studio", scope }: { heading?: string; scope?: string } = {}) {
  // `scope` = eine Landing (z. B. "urlaub") → eigener Card-Blob, getrennt von der Standardkarte.
  ...
  const load = () => fetch(`/api/bella-carousel?model=${encodeURIComponent(modelId)}${scopeQS}`, { headers: { "x-try-look-admin-pin": pin() } }).then(r => r.json()).then(d => { setSlides(d.slides ?? []); setBackup(d.backup ?? { count: 0, savedAt: "" }); setDirty(false); }).catch(() => {});
  ...
  const loadPreview = (cust: string) => {
    fetch(`/api/bella-card-preview?model=${encodeURIComponent(modelId)}${cust ? `&customer=${encodeURIComponent(cust)}` : ""}${scopeQS}`, { headers: { "x-try-look-admin-pin": pin() } })
  ...
  const post = (payload: object) => fetch("/api/bella-carousel", { method: "POST", headers: authH(), body: JSON.stringify({ ...payload, model: modelId, scope }) }).then(r => r.json());
    ...
    <div className="lb-theme mt-8 rounded-2xl border border-dashed border-amber-400/50 bg-white p-4">
      ...
      <div className="mt-2 rounded-xl border border-amber-400/20 bg-amber-400/[0.04] p-3">
        ...
        <textarea value={imgCaption} onChange={e => setImgCaption(e.target.value)} placeholder="Story (English) — leer lassen = KI schreibt sie"
          className="mt-2 h-16 w-full rounded-lg border border-white/15 bg-white/[0.04] px-2.5 py-1.5 text-[13px] font-medium text-white outline-none placeholder:text-white/50 focus:border-amber-400" />
        {stagedImg ? (
          <div className="mt-2">
            {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <input value={title} onChange={e => setTitle(e.target.value)} onBlur={() => title !== slide.title && onUpdate({ title })} placeholder="Titel (optional)"
            className="h-8 w-full rounded border border-white/15 bg-white/[0.04] px-2 text-[12px] font-bold text-white outline-none placeholder:text-white/50 focus:border-amber-400" />
