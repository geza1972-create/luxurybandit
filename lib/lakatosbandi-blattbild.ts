...
import { POSTER, POSTER_FORMATE } from "@/lib/lakatosbandi-poster";
...
export async function blattBildBauen(a: DruckAngaben & { dpi?: number }): Promise<Uint8Array> {
  ...
  type Zeile = { art: "text" | "sperr" | "qr"; inhalt: string; font: Schrift; groesse: number; farbe: string; sperre: number; danach: number; satz?: boolean };
  const zeilen: Zeile[] = [];
  const nameGroesse = cqw(P.name.breit);
  const nameText = a.leben ? `${a.name ?? ""}   ${a.leben}` : (a.name ?? "");
  if (a.name) zeilen.push({ art: "sperr", inhalt: nameText, font: serif, groesse: nameGroesse, farbe: f.tinte, sperre: nameGroesse * P.name.sperre, danach: cqw(P.luft) });
  if (a.titel) zeilen.push({ art: "text", inhalt: a.titel, font: kursiv, groesse: cqw(P.titel.breit), farbe: f.tinte, sperre: 0, danach: cqw(P.titel.luftUnten) });
  if (a.text) {
  ...
  if (a.recht) zeilen.push({ art: a.qrZiel ? "qr" : "text", inhalt: a.recht, font: serif, groesse: cqw(P.recht.breit), farbe: f.leise, sperre: 0, danach: cqw(P.luft) * 0.4 });
  if (a.nummer) zeilen.push({ art: "text", inhalt: `Licență ${a.nummer} · uz personal`, font: serif, groesse: cqw(P.recht.breit), farbe: f.leise, sperre: 0, danach: 0 });
  ...
  const feldHoehe = Math.min(cqw(P.bild.hoch * 1.4142) - cqw(P.randOben), Math.max(1, obenY - (untenY + blockHoehe + cqw(P.bild.luftSchrift))));
  const feldBreite = innen - 2 * cqw(P.bild.randSeite);
  /* Wie auf dem Schirm: volle Feldbreite, Überhang unten weg. */
  const geschnitten = await aufFeldSchneiden(a.bild, feldBreite, feldHoehe);
  const wB = geschnitten.breit;
  const wH = geschnitten.hoch;
  const skala = Math.min(feldBreite / wB, feldHoehe / wH);
  const bw = wB * skala;
  const bh = wH * skala;
  /* Im Bild zählt y von OBEN — die Oberkante liegt am Feldrand. */
  const werkX = mitte - bw / 2;
  const werkYvonOben = H - obenY;
  ...
  const werk = await sharp(Buffer.from(geschnitten.bild))
